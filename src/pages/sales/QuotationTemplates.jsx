...
import { getQuotationTemplates, deleteQuotationTemplate, createQuotationTemplate, updateQuotationTemplate } from '../../stores/salesStore'
import DataTable from '../../components/DataTable'
import Modal, { ModalFooter } from '../../components/Modal'
import FormInput, { FormTextarea, FormSelect } from '../../components/FormInput'
import { useToast } from '../../components/Toast'

function QuotationTemplates() {
        ...
        if (formData.default) {
            templates.forEach(t => {
                if (t.id !== (editingTemplate?.id) && t.default) {
                    updateQuotationTemplate(t.id, { default: false })
                }
            })
        }

        if (editingTemplate) {
            updateQuotationTemplate(editingTemplate.id, formData)
            toast.success('Template updated')
        } else {
            createQuotationTemplate(formData)
    ...
    const confirmDuplicate = () => {
        const newTemplate = createQuotationTemplate({
    ...
    const confirmDelete = () => {
        if (deleteConfirm) {
            deleteQuotationTemplate(deleteConfirm.id)
    ...
    const handleSetDefault = (template) => {
        templates.forEach(t => {
            updateQuotationTemplate(t.id, { default: t.id === template.id })
        })
    ...
    return (
        <div className="page">
            <motion.div className="page-header" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
                <div>
                    <h1 className="page-title"><span className="gradient-text">Quotation</span> Templates</h1>
                    <p className="page-description">Create reusable quotation templates for faster quote generation.</p>
                </div>
            ...
            <Modal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditingTemplate(null); resetFormData() }} title={editingTemplate ? 'Edit Template' : 'New Template'} size="large">
                ...
                <FormTextarea label="Default Terms" value={formData.terms} onChange={(e) => setFormData({ ...formData, terms: e.target.value })} rows={3} placeholder="Payment terms, delivery conditions, etc." />
                <FormTextarea label="Default Notes" value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} rows={2} placeholder="Thank you message, contact info, etc." />
